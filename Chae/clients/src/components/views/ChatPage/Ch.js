    ...
    return (
        <div className="chat-container">
            <div className="chat-header">
                <h2>참여 방: {room}</h2>
                {room && <button className="leave-room-button" onClick={leaveRoom}>나가기</button>}
                <button className="change-nickname-button" onClick={changeNickname}>닉네임 변경</button>
                <div className="room-buttons">
                    {rooms.map((roomInfo) => (
                        <div key={roomInfo.roomName}>
                            <button className="room-button" onClick={() => joinRoom(roomInfo.roomName)}>
            ...
            <input
                ...
                placeholder="방 이름"
            />
            <button className="chat-button" onClick={createAndJoinRoom}>방참가</button>
            ...
            <div className="chat-messages">
                <div>
                    {messages.map((msg, index) => (
                        <div className="chat-message" key={index}>{msg.nickname}: {msg.text}</div>
                    ))}
                </div>
                <div className="chat-input-container">
                    ...
                    />
                    <button className="chat-button" onClick={sendMessage}>보내기</button>
                </div>
            </div>
        </div>
    );
