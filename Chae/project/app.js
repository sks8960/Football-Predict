...
const { getTeamFixtureStatistics } = require('./getTeamFixtureStatistics');
app.set('views', './views');
...
app.get('/search_process', (req, res) => {
    ...
    db.query(`select * from test where referee=?`, [queryData.name], function (error2, topic) {
        ...
        var id = topic[0].id;
        /*console.log(id);*/
        db.query(`select * from test where id = ?`, `${id}`, function (error3, a) {
            console.log(a[0]['referee']);
        })
    })
}); //원하는 심판 

app.get('/stastics_process', (req, res) => {
    var _url = req.url;
    var queryData = url.parse(_url, true).query;
    getTeamFixtureStatistics(queryData.fixture)
        .then((statistics) => {
            res.end(JSON.stringify(statistics));
        })
        .catch((error) => {
            console.error(error);
            res.render('state', { statistics: {} });
        });
});

app.get('/search_team_process', (req, res) => {
    var _url = req.url;
    var queryData = url.parse(_url, true).query;
    const team = JSON.parse(JSON.stringify(queryData));
